...
{
    public partial class MobileGameInput : System.Web.UI.Page
    {
        ...
        protected void gvMobileGameInput_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            Response.Redirect("MobileGameInput.aspx?mode=android", true);
        }
        ...
        protected void gvMobileGameInput_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            esra_summary.Text = game.esra_summary;
            ...
            foreach (game_version version in gameVersion)
            {
                ...
                tempVersion.isSourseAvalable = version.is_source_available != null && (bool)version.is_source_available
                    ? 1
                    : 0;
            ...
            if (gameInfo.img != null)
                foreach (var s in gameInfo.img)
                {
                    if (!string.IsNullOrEmpty(s))
                        pnl_OldPic.InnerHtml += $"<div class=\"imgContainer\">" +
                                                $"<img class=\"gameLogo\" src=\"/uploads/picture/game/icons/{gameInfo.gameId}/{s}\">" +
                                                $"<div><span class=\"js-delPic\">حذف</span><span>{s}</span></div>" +
                                                $"</div>";
                }
            txtPicName.Text = game.icon;
