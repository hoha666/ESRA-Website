$(document).ready(function () {
    ...
    $(window).resize(function () {
        ...
        if ($(window).width() <= 828) {
            $('.navTop> div,.navTop').css('height', 'auto');
        } else {
            if ($(window).scrollTop() > 50)
                $('.navTop> div,.navTop').css('height', '40px');
            else
                $('.navTop> div,.navTop').css('height', '79px');
    ...
    $('#cpBodyMiddle_btnSearch').on('click', function () {
        var hfSearchGenres = "", hfSearchAges = "", hfSearchPlatform = "", hfSearchCategory="";
        ...
        $('#cpBodyMiddle_ddlCategory').find('.js-active').each(function () { hfSearchCategory += ($(this).attr('id') + ',') });
        ...
        $('#cpBodyBottom_hfSearchCategories').val(hfSearchCategory);
    });
    ...
    $('.gameLargLogoHolder img:not(:first-child)').on('click', function () {
        var firstChild = $('.gameLargLogoHolder img:first-child');
        var currentChild = $(this);
        var currentChildIMg = currentChild.attr('src');
        var firstChildIMg = firstChild.attr('src');

        firstChild.fadeOut(function () {
            firstChild.attr('src', currentChildIMg);
            firstChild.fadeIn();
        });
        currentChild.fadeOut(function () {
            currentChild.attr('src', firstChildIMg);
            currentChild.delay(300).fadeIn();
        });
    });
});

$(document).ready(function () {
    //$('.gameLargLogoHolder img').click();
    var offset = 20;
    var duration = 300;
    ...
    $(window).scroll(function () {
        ...
        if ($(this).scrollTop() > menuTopOffset) {
            if ($(window).width() <= 845) {
                $('.navTop> div,.navTop').stop().animate({
                    height: 'auto'
                }, 300);
            } else {
                $('.navTop> div,.navTop').stop().animate({
            ...
            if ($('.navTop img').attr("src") != "/IMG/logoN.png")
                $('.navTop img').stop().fadeOut(100, function () {
                    $(this).attr("src", "/IMG/logoN.png").css('height', '25px').fadeIn(300);
                });
            ...
            if ($('.navTop img').attr("src") != "/IMG/logo.png")
                $('.navTop img').stop().fadeOut(100, function () {
                    $(this).attr("src", "/IMG/logo.png").css('height', '62px').fadeIn(300);
                });
            $('.hamburger').stop().animate({
                padding: '15'
            });
